        ...
        if(playlistmode){
          ...
          stage.addEventListener("stagemousedown", function(e) {
            ...
            stage.addEventListener("stagemouseup", function(){
              ...
              shape = new createjs.Shape();
              stage.addChild(shape);
              for(i=0;i<playlistsong.length-3;i=i+2){
                shape.graphics.beginStroke(colorw).setStrokeStyle(size, "round",0,10,true).moveTo(playlistsong[i],playlistsong[i+1]).lineTo(playlistsong[i+2],playlistsong[i+3]);
              }
              stage.update();
              init();
            });
